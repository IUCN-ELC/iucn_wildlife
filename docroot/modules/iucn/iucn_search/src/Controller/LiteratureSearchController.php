...
class LiteratureSearchController extends DefaultSearchController {
  ...
  public function searchPage() {
    ...
    try {
      $result = self::getSearch('literature', ['sort' => 'field_date_of_text', 'sortOrder' => 'desc'])
      ...
      $numFound = $this->formatPlural($found, 'Found 1 literature', 'Found @count literatures');
      ...
      foreach ($sorts as $key => $sort) {
        if ($activeSort == $sort['field'] && $activeOrder == $sort['order']) {
          ...
          $url = Url::fromRoute('iucn.search.literature', [], ['query' => $getCopy]);
      ...
      if (empty($results)) {
        ...
        $results = [
          ...
          '#type' => 'container',
          [
            '#attributes' => ['class' => ['ecolexicon', 'ecolexicon-literature']],
            '#tag' => 'span',
            '#type' => 'html_tag'
          ],
          [
            ...
            '#tag' => 'h3',
            '#type' => 'html_tag',
            '#value' => $this->t('No literatures found.')
          ],
          [
            '#attributes' => ['class' => []],
            '#tag' => 'p',
